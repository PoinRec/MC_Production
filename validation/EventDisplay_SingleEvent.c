...
void EventDisplay_SingleEvent(const char * fname, int evtID)
{
    ...
    evty = vtx_z;
    TMarker mbarrel(evtx,evty,29);
    mbarrel.SetMarkerColor(kRed);
    mbarrel.Draw();
    evtx = -vtx_y;
    evty = max_z+max_r-vtx_x;
    TMarker mtop(evtx,evty,29);
    mtop.SetMarkerColor(kRed);
    mtop.Draw();
    evtx = -vtx_y;
    evty = -max_z-max_r+vtx_x;
    TMarker mbot(evtx,evty,29);
    mbot.SetMarkerColor(kRed);
    mbot.Draw();
